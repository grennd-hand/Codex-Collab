...
房主批准 → 选择任务 → 发布历史和文件 → 已批准成员读取 → 主人授予写权限 → Host
...
- `../`、绝对路径跳出和符号链接逃逸均被拒绝；
- 写文件时预期哈希变化会产生冲突；
- Windows 写入使用真实 replace-with-backup，并在原子替换前出现外部修改时回滚；
...
- 已批准协作者的指令可以进入 Host 队列，但继承主人的 Codex 审批/访问模式；
...
- 默认只读成员不能保存，主人授权后可保存项目文件，`.codex` 仍只读；
