...
- 已批准成员默认只读查看安全文本文件，主人可按成员授予项目文件写权限；
...
9. 已批准成员可以查看导入记录和文件；项目写入默认关闭，只能由主人按成员开启；待批准
   成员及非房主不能发布或选择任务。
...
- IDE 第一阶段支持现有安全 UTF-8 文本文件的读取和保存，尚未提供新建、重命名、删除、
  终端、调试器、扩展系统和 Git worktree 合并队列；
