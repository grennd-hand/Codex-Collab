...
直接显示，每两段处理说明之间相邻的正常完成/停止命令收进一个“运行了多个命令”折叠项，正文顺序
不变；只有当前命令保持展开，失败命令继续单独展开；嵌套命令会话在轮询期间沿用原始命令，明确完成
输出或退出码到达后立即折叠。
Beta 10 已覆盖安装并通过真实 Renderer 动态探针，
...
- [x] 后续共享 UI 明确按文字区间折叠：两段处理说明之间的相邻历史命令形成一个折叠项，不跨正文
  合并；视觉文案为“运行了多个命令”，精确数量保留在可访问名称中，当前命令与失败命令仍分别展开。
