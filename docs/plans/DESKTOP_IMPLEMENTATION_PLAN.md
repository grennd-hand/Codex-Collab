...
退出协调器却没有完成。`v0.1.0-beta.4` 已修复托盘入口；`v0.1.0-beta.5` 已加入 Explorer 双向
缩放修复。`v0.1.0-beta.6` 已在本机升级安装，安装 ASAR 与候选一致，并修复完成回复仍转圈、
...
右键退出的最终点击验收仍待主人确认。`v0.1.0-beta.11` 又把活动时间线改为 Codex 式正文流：处理说明
直接显示，每两段处理说明之间相邻的历史命令收进一个“运行了多个命令”折叠项，正文顺序
不变；只有当前命令保持展开，失败命令也留在原文字区间内默认折叠，并在折叠行显示失败数量；嵌套
命令会话在轮询期间沿用原始命令，明确完成输出或退出码到达后立即折叠。
Beta 12 已进一步统一终态历史的展开内容：“已处理”“失败”“已停止”展开后也使用正文与分段命令组，
...
- [x] `v0.1.0-beta.6` 已将最终回复作为当前 turn 的终态证据，陈旧 GET 不再覆盖 Realtime 的
  `completed`，Desktop 与 MCP 统一使用本机 profile 的 Host 状态目录；本机旧 Host 已停止，
  新安装实例的 lock/endpoint 收敛到同一 PID，并通过 18 工具 MCP 握手。
- [x] 当前源码已补齐 stopped 过程态：非活动任务的未决命令显示“已停止”，活动任务中只有最后
  一个步骤可显示运行；完成任务仍按最终总结折叠。相关 44 项 focused tests 与全仓门禁已通过；
  安装后可访问性树确认旧步骤不再显示 Spinner，展开正文也不再声称后台仍在运行。
...
- [x] 后续共享 UI 明确按文字区间折叠：两段处理说明之间的相邻历史命令形成一个折叠项，不跨正文
  合并；视觉文案为“运行了多个命令”，精确数量保留在可访问名称中，只有当前命令展开。
  `dbca79b` 的跨正文合并版本已由 `c11976e` 替换，修正版线上资产与真实 Realtime/终态闭环已验证。
...
- [x] 已安装 Renderer 动态验证：运行中的 `npm run typecheck` 只有一条展开命令并显示真实命令；
  ...
  原陈旧 `rg --files` 命令不可见。
- [x] `v0.1.0-beta.11` 已按提交 `48f710e` 重新生成并覆盖安装；安装 ASAR 与候选均为
  `0F66F0EB7706513E05618AEF02AAB31D33397B10A617AC03B52C82EBD88004F1`，安装包 SHA-256 为
  `A86002303E85247016263E9AD1A63C64D4DAE243016D8B92A9D5D4AEA935C3D1`。已安装 Renderer DOM
  证明正文与命令组按原始时间顺序交替，所有命令组 `aria-expanded=false`；两个历史失败命令也为
  `collapsed`，没有失败命令默认展开。
- [x] `v0.1.0-beta.12` 已按提交 `6f246e6` 生成并覆盖安装；安装 ASAR 与候选均为
  `65F13B6323BC74498F7EC9AC7DEC2C028F65BA84DDB19E3A4E317B54A1659D74`，安装包 SHA-256 为
  `02168BDC1B69DDB79030D95E1673FBBDE5CBC1C6C7A55339D6940838F4C58780`。已安装 Renderer 中点击
  “已处理”后验证为 2 个折叠命令组、1 段正文、0 条裸命令、0 个错误的继续处理提示。
- [x] Beta 13 源码已使用同一份 9 尺寸 32-bit ICO 驱动窗口、托盘、快捷方式、安装器和卸载项；
  图标为柔和白底、深靛蓝双面板轮廓，没有蓝点、渐变或第二套装饰色。
- [x] Beta 13 源码已把协作/活动、项目文件和 Codex 任务扁平为三个稳定 key 的可换位面板；
  专用拖动柄、左右移动按钮、键盘分隔条、双击复位、最小宽度和 task-scoped 持久化均有回归测试。
- [x] `v0.1.0-beta.13` 已按提交 `7da252a` 生成并覆盖安装；安装包 SHA-256 为
  `92113CB3928B2BA99B9FB0DD2FD868B128FE7EF1AF7003EA8CCE46E4757F9CDA`，安装 ASAR 与候选均为
  `D1D2FF0028EEFE4DEFCE3F8DC3B202FF73BA29BA8465C8FE7EE9405FB4818525`。安装后只有一个主人主进程
