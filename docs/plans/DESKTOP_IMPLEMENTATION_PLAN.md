...
直接显示，正常完成/停止的历史命令统一收进一个“运行了多个命令”折叠项，只有当前命令保持展开，
失败命令继续单独展开；嵌套命令会话在轮询期间沿用原始命令，明确完成输出或退出码到达后立即折叠。
...
- [x] 后续共享 UI 已把被正文分隔的正常历史命令也合并为唯一折叠项；视觉文案为
  “运行了多个命令”，精确数量保留在可访问名称中，当前命令与失败命令仍分别展开。该变更随提交
  `dbca79b` 部署到 Primary，线上 bundle 与真实 Realtime/终态闭环已验证。
