...
>
> 当前可运行 MVP 的真实状态以 [PROJECT.md](../reference/PROJECT.md) 和
> [TASK_PLAN.md](./TASK_PLAN.md) 为准；2026-07-28 的代码审计见
> [ARCHITECTURE_AUDIT_2026-07-28.md](../audits/ARCHITECTURE_AUDIT_2026-07-28.md)。本计划描述从最小闭环
> 继续演进到完整 v1 的目标。
>
...
- 公网邀请链接，无需双方安装 VPN
- 浏览器设备密钥＋邀请人手动批准
- 邀请人手动选择共享项目；只开放根目录内通过扩展名 allowlist、ignore、私有目录和敏感
  内容策略筛选的文件
- 高风险操作需要邀请人审批
...
下面的 Windows Host Agent、加密事件存储、PWA、device signing、双 Worktree 等属于目标形态，
并非当前仓库全部已采用。当前 MVP 是 TypeScript 插件 Host + Node Relay + SQLite + React
Dashboard；Relay 会保存经过过滤的有界文本快照。以下状态必须分开阅读：

| 分类 | 内容 |
| --- | --- |
| 当前 MVP | MCP/Host worker、Relay/SQLite、房间/邀请/批准、按 task 历史、Monaco 与租约文件操作 |
| 已采纳但未闭环 | 权限收紧、崩溃幂等、撤销/轮换、配额、备份、浏览器 E2E |
| 远期/待决 | 独立 C# Host、E2EE 历史、PostgreSQL/SignalR、PWA device key、双 Worktree |
...
目标形态下，主机离线时文件管理区显示离线状态且不保存云端文件快照。当前 MVP 与该目标
不同：Relay 会保存主人 Host 过滤、限量发布的文本文件目录/内容快照；这是当前事实而非
端到端加密或零云端快照承诺。
...
目标 v1 不包括公开注册、付费、多租户市场、陌生用户共享、手机原生应用或长期云端文件
快照。当前 MVP 已使用 Relay 上的过滤、有界文本快照，未来若删除该能力必须先完成迁移和
离线体验设计，不能把目标描述误作当前实现。
