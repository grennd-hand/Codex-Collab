...
├── app/                    Dashboard controller, shell and orchestration views
├── features/               collaboration plus grouped session/timeline/workspace domains
├── ide/                    changes, editor, explorer, shell and task/workspace state
├── layout/                 split-pane and workspace panel geometry
└── shared/                 low-level browser/API utilities

apps/desktop/
├── src/main.ts             Electron process entrypoint
├── src/preload.cts         enumerated Desktop renderer bridge
├── src/{app,credentials,host,ipc,relay,release,security}/
├── renderer/{app,panes,shell,styles}/
└── renderer/{index.html,main.tsx} stable Vite entrypoints

native/host-ipc/src/
├── main.rs/lib.rs          crate entrypoints
├── auth.rs/frame.rs        authenticated framed transport
├── pipe.rs/security.rs     Named Pipe and Windows ACL/SID boundary
└── instance.rs/protocol.rs single-instance ownership and typed method admission
...
├── http/                  payload, invite and Codex option adapters
├── realtime/              one-time realtime tickets
├── security/              bearer token primitives
├── testing/               shared store test support
...
├── workspace-sync-worker.ts Host Worker process entrypoint
├── app-server/            Codex facade, JSON-RPC transport, parsing and submission mapping
├── codex/                 Codex command delivery and Desktop IPC adapter
├── host/                  Host application/runtime/service and authenticated IPC
├── mcp/                   declarative MCP tool catalog
├── persistence/           local profile, durable outbox and recovery receipts
├── relay/                 typed Relay clients and HTTP transport
├── sync/                  workspace synchronization and admission orchestration
└── workspace/             roots, snapshots, file operations and sandbox/CAS boundary
```

The earlier dashboard extraction, Relay route split, plugin parser/transport split and plugin domain
layout are implemented. The plugin source root is guarded so only its two process entrypoints can
remain there. Current structural debt is different: Dashboard feature imports do not yet match the
intended dependency graph, and Relay storage modules are connected by a 14-level inheritance chain.
These are addressed incrementally; security checks and transactions stay at their real boundary
