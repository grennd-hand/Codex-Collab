...
- 当前源码保证协作者时间线的末次增量不会被实时刷新限流丢弃；终态事件会等待 Host 最终历史，
  并把刷新中的重复 history 请求合并为一次尾随重取。在任务终态或 Realtime 重连后还会对账指令
  状态，避免手动停止或已完成任务继续显示运行中、缺少最终总结；
- 已完成任务把执行过程、Codex 最终总结和结构化文件变更显示为一个结果块；执行步骤默认折叠，
  总结与耗时保持可见，文件列表先显示前三项且可继续展开；
- 活动任务使用 Codex 式正文流：处理说明直接显示，每两段处理说明之间相邻的历史命令折叠为一个
  “运行了多个命令”批次，不跨正文重排；仅当前运行命令展开，失败命令默认折叠并保留失败标记；
...
当前本机已进一步升级到 `v0.1.0-beta.11`：正文和其后执行的命令组严格按时间交替，不跨正文合并；
成功、停止、失败的历史命令均默认折叠，失败数量保留在折叠行，只有当前运行命令展开。最终安装
ASAR SHA-256 为 `0F66F0EB7706513E05618AEF02AAB31D33397B10A617AC03B52C82EBD88004F1`，
manifest 对应提交 `48f710e`。
`v0.1.0-beta.12` 又统一了终态历史路径：“已处理”“失败”“已停止”展开后也使用相同的正文与
分段命令组，不再显示逐命令列表；终态展开不会显示继续处理提示。安装 ASAR SHA-256 为
`65F13B6323BC74498F7EC9AC7DEC2C028F65BA84DDB19E3A4E317B54A1659D74`，manifest 对应提交
`6f246e6`。
