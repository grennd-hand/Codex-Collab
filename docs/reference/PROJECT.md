...
- 活动任务使用 Codex 式正文流：处理说明直接显示，正常完成/停止的历史命令统一折叠为一个
  “运行了多个命令”批次，仅当前运行命令展开，失败命令保持单独展开；嵌套长命令在轮询期间
  保留原始命令，明确完成输出或退出码到达后立即折叠；
...
反馈闭环先于 2026-07-31 随提交 `6904723` 部署，单一历史命令折叠又随提交 `dbca79b` 部署到
Primary Relay；Guest 保持原 release、原容器和原首页资产，共享 Caddy 未重启，也未作为桌面端
验证环境。详细状态见
[主人桌面端实施计划](../plans/DESKTOP_IMPLEMENTATION_PLAN.md)。
