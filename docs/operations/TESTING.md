...
node scripts/probes/probe-mcp.mjs
npm audit --audit-level=high
```

MCP 验证必须真实完成 `initialize` 和 `tools/list`。仅看到进程运行或 HTTP 响应不算通过。
`validate:plugin` 通过仓库内 `scripts/plugin/validate-plugin.mjs` 入口定位验证器，不再固化某个
Windows 用户目录。CI 或非 Codex 环境可用 `CODEX_COLLAB_PLUGIN_VALIDATOR` 指向版本固定的
`validate_plugin.py`；找不到验证器时门禁失败关闭。
