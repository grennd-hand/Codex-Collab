...
const DEFAULT_WORKSPACE_HISTORY_PAGE_SIZE = 40;
const MAX_WORKSPACE_HISTORY_PAGE_SIZE = 100;
...
export class SessionStore {
  ...
  getWorkspaceHistoryPage(
    ...
    const limit = options.limit ?? DEFAULT_WORKSPACE_HISTORY_PAGE_SIZE;
    if (
    ...
    const history = this.parseHistory(state.history_json);
    const allItems = workspaceHistoryPageItems(history);
    let end = allItems.length;
    if (options.before) {
      const cursor = decodeWorkspaceHistoryCursor(options.before);
      if (
        cursor.sessionId !== sessionId ||
        cursor.threadId !== state.selected_thread_id
      ) {
        throw new ProtocolError(
          409,
